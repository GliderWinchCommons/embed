...
*/
	...
	// Copy table entries to struct in sram from high flash.
	  ret  = engine_idx_v_struct_copy_eng_man(&shaft_f.lc, shaft_f.cf.pparamflash);
...
//shaft_f.cf.hb_tdur = 1500;
	...
	// Convert to parameters to double since computations done in doubles
	 shaft_f.dpress_offset = shaft_f.lc.press_offset;
	 shaft_f.dpress_scale  = shaft_f.lc.press_scale;
	 shaft_f.cf.can_msg.id = shaft_f.lc.cid_msg;	// Outgoing CAN id: polled msg
	 shaft_f.cf.can_hb.id  = shaft_f.lc.cid_hb;		// Outgoing CAN id: heartbeat
	 shaft_f.cf.canid_poll = shaft_f.lc.code_CAN_filt[0]; // Incoming poll CAN id
