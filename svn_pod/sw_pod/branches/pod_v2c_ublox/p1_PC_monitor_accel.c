...
char cAccelMsgFlag  = 0;	// One message switch
/******************************************************************************
...
void p1_PC_monitor_accel(void)
{
	...
	case 0:
		...
		{ // Here, logging is OFF
			...
			if (cAccelMsgFlag == 0)
			{
				USART1_txint_puts(" Accelerometer logging is OFF\n\r  hit 'y<enter>' to toggle ON/OFF\n\r  hit 'p<enter>' to save status at next 'sleep'\n\r ");	USART1_txint_send();//  (@1)
				cAccelMsgFlag = 1;
			}
			return;
		}
