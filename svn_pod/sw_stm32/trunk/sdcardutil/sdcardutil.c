...
//			if (lb.ct > 0)				// Check if we have a completed line
		...
		{ // Here we have a pointer to the line and a char count
			...
			case 'a':
				uiMax = 0; ulAve = 0;
				printf ("pass.w.block num.max(usec).ave(usec)\n\r");USART_txint_send();
				NEXT_STATE(a_funct);
				data_mask = 0;
...
void a_00(void)
{
	...
	sdcard_ll_cmd(CMD24, block_no, (void *)caw_realdata);
...
void a_01(void)
{
	...
	if((block_no % bpp) == 0)
	{
		...
		{
			uiSysDur = ulAve / (bpp*10);
			printf("\r\n%u.w.%07u.%06u.%06u> ", pass_no, block_no, uiMax/(sysclk_freq/1000000),uiSysDur/(sysclk_freq/1000000) );
			ulAve = 0;uiMax = 0;
		}
		USART_txint_send();
	}
...
void b_00(void)
{
	sdcard_ll_cmd(CMD17, block_no, (void *)caw_realdata);
