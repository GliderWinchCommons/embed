...
'usartx_rxint_rxisrptradv.c' (loacted in usartx_rxint.c)
...
{	
	...
	{ // Here, EOL, so move to next line buffer.
		...
		*pUSARTcbrx->prx_ctary_now_i = (pUSARTcbrx->prx_now_i - pUSARTcbrx->prx_begin_i);
		...
		if (pUSARTcbrx->prx_now_i < (pUSARTcbrx->prx_begin_i + pUSARTcbrx->rx_ln_sz + 4) )
		{ // Here, there is room for the zero
			...
			*(unsigned int *)(pUSARTcbrx->prx_now_i) = uiRTCsystemcounter;	// Add 32 bit counter
		}
		else
		{
