	...
	case 0:	
		...
		/* Let's check for a continuous duration above the threshold */
			...
			for ( i = 0; i < PKT_AD7799_TENSION_SIZE; i++)
			{
				...
				ntemp = converttograms(pt->tension[i]);	// Convert to grams for ease of use

				if (ntemp < TENSION_ACTIVE_THRESHOLD) // Tension too low to be an active launch?
			...
			/* If 1/2 the entries were above the threshold, then count the number of such packets */
			if (sTensionEntryCtr == (PKT_AD7799_TENSION_SIZE/2))
			{
				sTensionPktCtr += 1;
...
//shuttest('$');
			...
			/* Write SD card with updated launch times, if necessary */
			p1_launchtime_close();

			/* Write SD card with updated pushbutton times, if necessary */
			p1_pushbuttontime_close();
		}
		break;
...
//		while (sPreTickReset != 0);	// Wait if there is a tick adjust in-progress
		...
		Powerdown_to_standby_v2b();	// Final wrapup and go into STANDBY (@5). (@7)
