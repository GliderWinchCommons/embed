...
* Date               : 12/09/2016
* Board              : F4-Discovery
* Description        : USB PC<->CAN gateway--FTDI version, with launchparameter test
*******************************************************************************/
/* 
TODO 
...
/*  WARNING--if UXPRT changes change definition in cmd_c.d ### */
...
/* ---------------------- Set up pins ------------------------------------------------------------- */
	...
	pushbutton_pinsetup();	// Setup PA0 for User pushbutton input
/* ---------------------- Set usb ----------------------------------------------------------------- */
...
//	bsp_uart_int_init_number(USTDO, 115200,  256,  256, 0x10, 1, 0);
	...
	bsp_uart_int_init_number(UXPRT, 115200, 1024,  512, 0x30, 0, 0);
...
/* ---------------------- Let the hapless Op know it is alive ------------------------------------ */
	...
	for (i = 0; i < 1; i++) 
	{
		...
		xprintf(UXPRT,  "\n\n\rLAUNCH PARAMETER TEST\n\r"
			"F4 DISCOVERY GATEWAY FTDI w LINKED LIST CAN1 & CAN2 DRIVER: 12-09-2016  hw fp v0\n\r");
...
/* --------------------- Endless Polling Loop ----------------------------------------------- */
	while (1==1)
	{
		...
		/* ================ PC --> CAN ================================================================= */
		/* Poll launch parameter transfer routine (for time out checking) */
		cmd_c_do_msg(NULL);

		/* Check if msg from PC is ready */
...
xprintf(UXPRT,"TMP err: %d %u\n\r", tmp, yctr++);
				...
				*/
					cmd_c_do_msg(&canrcvbuf);	// Check msg for Launch parameter transfer
