...
printf ("\n### CANGATEBMS ### 04/17/2025\n\n");

printf("BAUDRATE is FIXED AT: %s baud\n",baudratesetting);

	/* Check that variables are the sizes that is expected. */
//	if (var_size_chk() != 0)
//	{ // Here, failed, so we are doomed!
//		exit(-1);
//	}
...
printf("%d %s\n",i, argv[i]);
	...
	{ // Here, yes.
		serialport = argv[1];
		if (argc == 3)
		{ // Here there were at least wo arguments on the command line and 2nd is presumed to be the file with the test msgs.
