...
*******************************************************************************/
/*
09/29/2024 Update for F446 bmsmot CAN loading

01/21/26 change for (int to define int ahead of for)
...
#define USLEEP 10 //420// usec pacing of sending
...
enum STATES_MSG
{
	...
	STATE_MSG_REQ_CRC,/* 2 */	
	STATE_MSG_CHKSUM, /* 3 */
	STATE_MSG_ADDR_RESPONSE,/* 4 */
	STATE_MSG_DATA_END_OF,	/* 5 */
};
...
void download_init(void)
{
	timertickctr = (2000/10); // 2000 ms startup jic
...
void download_canbus_msg(struct CANRCVBUF* p)
{
	download_time_chk(); // CAN msgs poll timer countdown
...
//				exit (-15);	
	...
	printf("Node CRC matches: 0x%08X : 0x%08X\n",xbin_crc, p->cd.ui[1]);
			/* NEXT STEP: Check that checksums match. */
				...
				timertickctr = (100/10);  // 550 ms timeout				
	...
	case STATE_MSG_CHKSUM:
		...
		{ // Node returned expected command code: LDR_CHKSUM
			if (p->dlc != 8)
			{
...
//xx:				
				send_U8nnnX4(CANnodeid,LDR_SET_ADDR_FL, xbin_addr);
				state_timer = STATE_TIM_SET_ADDR_TO; // Timeout takes action here
				timertickctr = (550/10);  // 550 ms timeout				
				state_msg = STATE_MSG_ADDR_RESPONSE;
...
//			download_settimeout((req_size/1500 + 1), 0); // 
			timertickctr = (2000/10);  // 990 ms timeout	
...
printf("LDR_NACK: bin_ct %d data_retry_ct %d\n",bin_ct, data_retry_ct);
			...
			{ // Execute a retransmission
				...
				timertickctr = (900/10);  // 900 ms timeout	
				send_CANnodeid_data();
				break;
			}
			else
...
//	if ((current_value.it_value.tv_nsec == 0) && (current_value.it_value.tv_sec == 0))
	if (timertickctr == 0)
	{ // Here, countdown timer has timed out
		switch (state_timer)
		{
		case STATE_TIM_IDLE:
			return;
...
//			download_settimeout(0, (50*MSEC)); // 50 ms timeout, JIC a HB from target is in progress.
			timertickctr = (50/10);  // 900 ms timeout	
	...
	// Suffix TO stands for timeout, i.e. failure to get a timely response
		...
		case STATE_TIM_REQ_CRC: // Request CRC from target node
			...
			timertickctr = (500/10);  // 900 ms timeout	
			break;
		...
		case STATE_TIM_SET_ADDR_TO: // Timeout waiting load address response
			...
			send_U8nnnX4(CANnodeid,LDR_SET_ADDR_FL, xbin_addr);
			timertickctr = (500/10);  // 500 ms timeout	
			state_msg = STATE_MSG_ADDR_RESPONSE;
			state_timer_retry_ct += 1;
			if (state_timer_retry_ct > 20)
			{
		...
		case STATE_TIM_DATA_END_OF_TO: // Timeout waiting for response to CAN msg data burst
			...
			printf("## CRC: 0x%08X CHK: 0x%08X bldct: %d\n",crc,(unsigned int)binchksum,bldct*4);
			timertickctr = (500/10);  // 500 ms timeout	
			state_timer_retry_ct += 1;
			if (state_timer_retry_ct > 4)
			{
...
static int starttimer(void)
{
	...
	if (ret != 0)
	{
		printf("\ntimer thread startup unsucessful\n");
		return -1;
	}
	return 0;
}
