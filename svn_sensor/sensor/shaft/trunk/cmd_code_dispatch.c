...
unsigned int cmd_code_dispatch(struct CANRCVBUF* pcan, struct SHAFT_FUNCTION* p)
{
	...
	pcan->id = *p->cf.pcanid_cmd_func_r; // "r" = response
...
case CMD_GET_READING_BRD: // 37,'Send a reading for the code specified in byte [1] for board; common to functions');
	break;
case CMD_LAUNCH_PARM_HDSHK: // 38,	'Send msg to handshake transferring launch parameters');
   break;
case CMD_SEND_LAUNCH_PARM:  // 39,	'Send msg to send burst of parameters');
   break;
case CMD_REQ_HISTOGRAM: // 40, 'Request histogram be taken and sent');
  adc_histo_request(pcan);	// Trigger and save duration for accumulating data in histogram bins
   break;
case CMD_THISIS_HISTODATA: // 41, 'Histogram data item');
   break;
