...
/* Line buffer size is used a number of places to set a max limit on loops */
#define GPSUARTLINEBUFSIZE	132	// Line buffer size for UART receiving GPS sentences
...
struct ALLTIMECOUNTS
{
	...
	union LL_L_S		TM3;		// Time 3 tick count (8192/sec)	
...
//	int		nPolyOffset32;		// Polynomial computed osc error 32 KHz osc
	...
	int pad[3];	// Keep size the same 

};


#define GPSARRAYSIZE	20	// Size of SSMMHHDDMMYY
...
/* Packet for GPS fix data (3-D) */
#define PKT_GPSFIX_SIZE	GPSUARTLINEBUFSIZE	// Number of chars allowed for longest sentence
struct PKT_GPSFIX
{
	unsigned char id;		// Packet ID
	union LL_L_S 	U;		// 64 bit Linux time
	char 	c[PKT_GPSFIX_SIZE];	// Variable length raw GPS sentence
};

/* This is used by 'p1_PC_monitor_readout.c' which gets packets from the SD Card, prepares & sends them to the USART */
#define PACKETREADBUFSIZE	160	// Must be bigger than the largest packet
...
union PKT_ALL
{
	...
	struct PKT_GPSFIX  gpsfix;		// ID 6 GPS fix
};
...
#include "gps_shutdown.h"	// GPS power up/down via load-cell activity
...
#include "p1_launchtime_sd.h"	// Launch time buffering
#include "p1_pushbuttontime_sd.h"// Pushbutton time buffering
...
#include "p1_PC_monitor_gps.h"	// Output current gps time settings to PC
#include "p1_PC_monitor_gpsfix.h"// Output current gps fix to PC
...
#include "pwrctl_v2b.h"		// Low power routines
...
#define INACTIVELOGGINGTICKCOUNT 5*3600*ALR_INCREMENT // Shutdown timeout counter reset count (seconds) (@2)) (@16) (@17)

/* This sets the deep sleep time */
// For 'shutdownflag' = 1   Normal deepsleep with periodic wake-up to adjust time for temperature
#define DEEPSLEEPTICKCOUNT1	    10*RTC_TICK_FREQ // Count for duration of deepdleep STANBDY (seconds * ticks/seconds (@2)) (@16)
// For 'shutdownflag' = 2   Battery low deepsleep--very long time, and depends on pushbutton to wakeup
#define DEEPSLEEPTICKCOUNT2	    -1	 // Count for duration of deepdleep STANDBY (seconds * ticks/seconds (@2)) (@16)
...
/* These 'unions' are used to read bytes into the correct order for the mult-byte AD7799 registers.
...
/* #### GPS shutdown timing #### */
// Timer is set by 'i' command, or load-cell activity detected.  Timeout -> GPS powered down.
#define GPSSHUTDOWNTIMERDURATION	5400	// Number of seconds to keep GPS powered when triggered by "activity"
//#define GPSSHUTDOWNTIMERDURATION	30	// Number of seconds to keep GPS powered when triggered by "activity" DEBUG

/* #### Tension related thresholds and timings #### */
//  'gps_shutdown' -- handling of load-cell, or "all out" high tension, triggers gps power, if not already on.
#define LOADCELLACTIVE 	800			// Difference in highest and lowest readings that indicates activity
#define LOADCELLALLOUT	30000			// At this level ("all out") or higher we want the gps ON...asap

// 'p1_shutdown' -- Each time launch threshold (multple packets) detected timer (5 hour) is reset.  Timeout -> sleep mode.
#define TENSION_ACTIVE_THRESHOLD	30000	// AD7799 reading for launch threshold tension
#define TENSION_ACTIVE_DURATION		4	// Number of tension packets above threshold needed to assure tension is a launch

// Launch readout--limit readout in case beginning of launch not detected
#define LAUNCHREADOUTMAXSECS	300	// Number of secs max readout if beginning of launch tension detect doesn't work
//#define LAUNCHREADOUTMAXSECS	60	// Number of secs max readout if tension detect doesn't work  DEBUG

// 'p1_launchtime' (end-of-launch), 'p1_PC_monitor_readout' (beginning-of-launch) tension thresholds for detection
#define LAUNCHTIMEREADOUT_TENSION_UP	100000	// Grams tension on way up
#define LAUNCHTIMEREADOUT_TENSION_DN	30000	// Grams tension on way down
...
// 'p1_launchtime' (end-of-launch), add to time after tension goes down by counting tension packets (0.250 sec/packet),
// and 'p1_PC_monitor_readout' (beginning-of-launch), add to time after tension goes down by counting tension packets (0.250 sec/packet)
#define LAUNCHTIMEREADOUT_DELAY	12	// Number of tension packets to count at beginning
