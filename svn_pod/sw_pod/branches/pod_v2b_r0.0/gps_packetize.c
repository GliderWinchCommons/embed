...
//		}
		...
		if ((cGPSng == 1) && ((GPIO_IDR(GPIOD) & (1<<14) ) != 0))	// GPS ready AND GPS power is on
		{
			if (GGAflag == 0) // Is the GGA height fix present?
			{ // Here yes.
				LEDonboard_ctl_turnon_ct(10, 1000,3);	// ON time, OFF time (ms), three flashes
			}
			else
			{ // Here no, but we are getting GGA time/
...
#define NOGPSDELAYCT	50000	// Number of pollings before switching to single flash of the LED
		...
		switch (cGPStype) // This is set in 'p1_initialization.c'
		{
...
//break;
			...
			if (uiGPSrtc == 1)
			{
				...
				{ // Here, the fixes will have settled down.		
					...
					{
					case GARMIN_18X_5_HZ:
						tmp = gpsfix_time_GGAsave(pgps);
						usGGAsavectr += 1;
						if (tmp >= 0) // GGA line?
							GGAflag = tmp; // Yes. 0 = OK; 1 = OK, but no height field
 ...
 * @param	: p = pointer to string with gps sentence (starting with '$')
 * @return	: 0 = OK, 1 = OK, but no height field, -1 = not GPGGA
 ******************************************************************************/
