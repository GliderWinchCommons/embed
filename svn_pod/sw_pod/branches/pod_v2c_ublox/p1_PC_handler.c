...
"g - monitor gps time setting \n\r",\
"h - monitor gps fix \n\r",\
"i - Trigger GPS power ON \n\r",\
"j - GPS power OFF \n\r",\
"k - launch time readout \n\r",\
"l - Toggle GPS sentences listing on-off\n\r",\
"m - Monitor load-cell data\n\r",\
"r - Readout data backwards in time (*)\n\r",\
"s - shutdown to sleep mode now\n\r",\
"q - pushbutton time readout \n\r", \
" \n\r",\
...
"x - stop and return to menu\n\r",\
"y - (when running a): toggle old/new accelerometer logging\n\r",\
...
void p1_PC_handler(void)
{
	...
	case 1:	// Waiting for input command
		...
		case 'g':	// Monitor gps time setting
			state_PC_handler = 'g';
			break;
		case 'h':	// Monitor gps fix
			state_PC_handler = 'h';
			break;
		case 'i':	// Trigger GPS power on
			state_PC_handler = 'i';
			break;
		case 'j':	// Turn GPS power off
			state_PC_handler = 'j';
			break;
		case 'k':	// Launchtime readout
			state_PC_handler = 'k';
			break;
		case 'l':	// Toggle GPS sentence listing
			state_PC_handler = 'l';
			break;
		case 'q':	// Launchtime readout
			state_PC_handler = 'q';
			break;
...
/* email 10/04/2011 02:53:58 PM: 2. Doing an "sdlog_flush()" before you start to read is a good idea. */
		...
		{ // Here, stop streaming
			...
			p1_PC_monitor_readout_launchtimes_reset();
		}
		else
		{ // Continue
			if (input_return == '*')
...
//p1_PC_monitor_readout();
	...
	case 'h':	// Monitor gps fix
		...
		{ // Continue
			RS232_ctl_reset_timer();	// Reset MAX232 timeout counter @2
			gps_monitor_flag = 1;		// Keep gps alive
			p1_PC_monitor_gpsfix();
		}
		break;
	
	case 'i':	// Trigger GPS power to come on for a timed duration (see gps_shutdown.c)
		if (input_return == 'x')
		{ // Here, stop streaming
			gps_monitor_flag = 0;		// Let MAX232 timeout
			state_PC_handler = 0;
		...
		{ // Continue
			RS232_ctl_reset_timer();	// Reset MAX232 timeout counter @2
			gps_monitor_flag = 1;		// Keep gps alive
			triggerGPSon(GPSSHUTDOWNTIMERDURATION);	// Set time duration (secs)
			gps_monitor_flag = 0;		// Let MAX232 timeout
			state_PC_handler = 0;
			USART1_txint_puts(" i GPS ON\n"); USART1_txint_send();

		}
		break;

	case 'j':	// Turn GPS power off now (rather than wait for timeout)
		if (input_return == 'x')
		{ // Here, stop streaming
			gps_monitor_flag = 0;		// Let MAX232 timeout
			state_PC_handler = 0;					
		...
		{ // One time 
			gps_powerdown();		// Shutoff GPS power, clear timer
			RS232_ctl_reset_timer();	// Reset MAX232 timeout counter @2
			gps_monitor_flag = 0;		// Let MAX232 timeout
			state_PC_handler = 0;	
			USART1_txint_puts(" j GPS OFF\n"); USART1_txint_send();	
		}
		break;
	...
	case 'a':	// Monitor accelerometer
		...
		{ // Continue
			/* Toggle accelerometer logging on/off */
			if (input_return == 'y')
			{
				strDefaultCalib.ACCELEROMETER_LOGGING ^= ~0; // Toggle accelerometer logging
				p1_logging_handler_init();	// Change "skip" table for skipping logging
				input_return = 'a';		// Kill multiple passes through this
			}
			/* Save accelerometer logging switch in calibration table */
			if (input_return == 'p') 	// 
			{
				USART1_txint_puts(" p = SAVE ACCELEROMETER ON/OFF\n\r"); USART1_txint_send();
				cCalChangeFlag = 1;	// We changed "calibration" so set flag to cause update when we shutdown ('s' command)
			}
			RS232_ctl_reset_timer();	// Reset MAX232 timeout counter @2
			p1_PC_monitor_accel();
		}
...
cmd_save = cmd;// Debugging
				...
				/* Check for a valid command (first char, except for 'd' 'f' 'k' commands) */
		...
		/* If the char is not in the following list of 'case's then output the "Huh" error msg */
				...
				case 'z':	// Toggle load-cell averaging
					break;
				case 'y':	// Toggle accelerometer logging
					break;
				...
				case 'k':	// Launchtime readout-- input line should have 'k x yyyy'
					if (strlb.ct >= SIZESLAUNCHTIME)  // (p1_PC_monitor_readout.h)
					{ // Check for a valid 'k n yyyy' commmand (n = 1 or 2; yyyy = 0 - x)
						if (convert_k_command(strlb.p) != 0)
						{ // Here something was wrong with the input fields
							cmd = 0;	// Consider it bogus and print error message
						}
						else
				...
				case 'q':	// Pushbutton time readout-- input line should have 'k x yyyy'
					if (strlb.ct >= SIZESLAUNCHTIME)  // (p1_PC_monitor_readout.h)
					{ // Check for valid 'q n yyyy' commmand (similar to 'k' command
						if (convert_q_command(strlb.p) != 0)
						{ // Here something was wrong with the input fields
							cmd = 0;	// Consider it bogus and print error message
						}
						else
