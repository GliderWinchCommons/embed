...
#include "db/idx_v_val.c" // #define generated by java from database

#ifndef NULL
#define NULL 0
#endif

/* Separate incoming hw filter lists by fixed algorithm */
#define FIFOBREAK 0x00E00000	   // CAN ID greater than this go in FIFO0
#define FIFOBREAKMASK 0xFFE00000 // High order bits for priority
...
int i;
for (i = 0; i < CANFILTMAX; i++)
{
	/* Check which priority group to place this canid */
	if ( (p->code_CAN_filt[i] & FIFOBREAKMASK) <= FIFOBREAK)
	{ // Here, high priority
		CAN_filter_build_fifo1(p->code_CAN_filt[i]);
	}
	else
	{ // All others
		CAN_filter_build_fifo0(p->code_CAN_filt[i]);
	}
}
